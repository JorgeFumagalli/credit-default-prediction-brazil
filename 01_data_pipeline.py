#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
==============================================================================
01_data_pipeline.py
TCC — Previsão da Inadimplência de Cartões de Crédito no Brasil

SCRIPT 1: EXTRAÇÃO, PREPARAÇÃO E DIAGNÓSTICOS ESTATÍSTICOS
==============================================================================

Este script consolida as etapas iniciais do pipeline:
    1. Extração das séries temporais do BCB/SGS
    2. Consolidação da base macroeconômica
    3. Preparação dos datasets FULL e EXCL
    4. Diagnósticos estatísticos da etapa de preparação

Saídas principais:
    - ./data/dados_consolidados_macro_credito.parquet
    - ./prepared/prepared_FULL.parquet
    - ./prepared/prepared_EXCL.parquet
    - ./results_preparation/*

Observações metodológicas:
    - As variáveis explicativas entram em nível (sem lags generalizados)
    - É criada apenas TARGET_lag1
    - Não há imputação por forward fill
    - O cenário EXCL remove 2019-01-01 a 2021-12-01
============================================================================== 
"""

# %% ==================== IMPORTAÇÕES ====================
import warnings
warnings.filterwarnings("ignore")

from functools import reduce
from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import requests
import statsmodels.api as sm

from scipy.stats import boxcox, norm, shapiro
from sklearn.linear_model import LinearRegression
from sklearn.preprocessing import StandardScaler
from statsmodels.stats.outliers_influence import variance_inflation_factor
from statsmodels.stats.stattools import jarque_bera

# Dependências opcionais
try:
    import pingouin as pg
    HAS_PINGOUIN = True
except Exception:
    HAS_PINGOUIN = False

try:
    from statstests.tests import shapiro_francia
    HAS_SHAPIRO_FRANCIA = True
except Exception:
    HAS_SHAPIRO_FRANCIA = False

try:
    from statstests.process import stepwise
    HAS_STEPWISE = True
except Exception:
    HAS_STEPWISE = False


# %% ==================== CONFIGURAÇÕES ====================
DATA_DIR = Path("./data")
PREP_DIR = Path("./prepared")
RES_DIR = Path("./results_preparation")

for directory in [DATA_DIR, PREP_DIR, RES_DIR]:
    directory.mkdir(parents=True, exist_ok=True)

START_DATE = "2011-03-01"
END_DATE = "2025-07-01"

DATE_COL = "data"
TARGET = "inadimpl_cartao_total"

SERIES = {
    "selic_mensal": 4390,
    "ibcbr_dessaz": 24364,
    "ibcbr_sem_ajuste": 24363,
    "inadimpl_cartao_total": 25464,
    "ipca_mensal": 433,
    "comprometimento_renda": 29034,
    "endividamento_familias": 29037,
}

FEATURES_RAW = [
    "selic_mensal",
    "ibcbr_dessaz",
    "ibcbr_sem_ajuste",
    "ipca_mensal",
    "comprometimento_renda",
    "endividamento_familias",
]

ADD_TARGET_LAG1 = True
EXCL_START = "2019-01-01"
EXCL_END = "2021-12-01"

TOPK_SCATTER = 8
STEPWISE_P = 0.05

SESSION = requests.Session()
SESSION.headers.update({
    "Accept": "application/json",
    "User-Agent": "Mozilla/5.0 (SGS-client; TCC Jorge Fumagalli)"
})


# %% ==================== ESTILO DOS GRÁFICOS ====================
def format_axes(ax, xlabel: str = "", ylabel: str = "", title: str = "", legend: bool = False):
    """Aplica formatação visual próxima ao padrão ABNT usado no TCC."""
    if title:
        ax.set_title(title, fontfamily="Arial", fontsize=12, color="black")

    ax.set_xlabel(xlabel, fontfamily="Arial", fontsize=11, color="black")
    ax.set_ylabel(ylabel, fontfamily="Arial", fontsize=11, color="black")

    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontfamily("Arial")
        label.set_fontsize(9)
        label.set_color("black")

    ax.grid(False)
    ax.set_facecolor("white")
    if ax.figure is not None:
        ax.figure.set_facecolor("white")

    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)

    for spine in ["bottom", "left"]:
        ax.spines[spine].set_linewidth(1.5)
        ax.spines[spine].set_color("black")

    if legend:
        ax.legend(frameon=False, prop={"family": "Arial", "size": 9})


# %% ==================== FUNÇÕES DE EXTRAÇÃO ====================
def _fmt(date_str: str) -> str:
    """Converte data para dd/mm/yyyy aceito pela API do SGS."""
    return pd.to_datetime(date_str).strftime("%d/%m/%Y")


def _fetch_sgs(host: str, codigo: int, start: str, end: str, timeout: int = 60) -> requests.Response:
    """Executa requisição HTTP para uma série SGS."""
    url = f"{host}/dados/serie/bcdata.sgs.{codigo}/dados"
    response = SESSION.get(
        url,
        params={
            "formato": "json",
            "dataInicial": _fmt(start),
            "dataFinal": _fmt(end),
        },
        timeout=timeout,
    )
    return response


def get_sgs(codigo: int, start: str = START_DATE, end: str = END_DATE, timeout: int = 60) -> pd.DataFrame:
    """
    Baixa uma série do SGS com fallback entre os hosts do Banco Central.
    """
    hosts = [
        "https://api.bcb.gov.br",
        "https://dadosabertos.bcb.gov.br",
    ]

    for host in hosts:
        try:
            response = _fetch_sgs(host, codigo, start, end, timeout)
            ctype = response.headers.get("Content-Type", "").lower()

            if response.status_code == 200 and "json" in ctype:
                data = response.json()
                df = pd.DataFrame(data)

                if df.empty:
                    return pd.DataFrame(columns=[DATE_COL, "valor"])

                df["valor"] = pd.to_numeric(
                    df["valor"].astype(str).str.replace(",", "."),
                    errors="coerce"
                )
                df[DATE_COL] = pd.to_datetime(df[DATE_COL], dayfirst=True)
                return df

        except Exception as exc:
            print(f"   ⚠️ Tentativa falhou em {host}: {exc}")
            continue

    raise RuntimeError(f"Falha ao baixar a série SGS {codigo}.")


def to_month_start(df: pd.DataFrame) -> pd.DataFrame:
    """Uniformiza datas para o primeiro dia do mês."""
    out = df.copy()
    out[DATE_COL] = out[DATE_COL].dt.to_period("M").dt.start_time
    return out


def extract_all_series() -> pd.DataFrame:
    """Extrai todas as séries configuradas e salva a base consolidada."""
    print("\n" + "=" * 80)
    print("BLOCO 1: EXTRAÇÃO DAS SÉRIES DO BCB/SGS")
    print("=" * 80)

    dataframes = []

    for name, code in SERIES.items():
        try:
            print(f"Baixando: {name} (SGS {code})...", end=" ")
            df = get_sgs(code, START_DATE, END_DATE)
            df = to_month_start(df).rename(columns={"valor": name})
            df = df[[DATE_COL, name]]
            dataframes.append(df)
            print(f"✅ {len(df)} observações")
        except Exception as exc:
            print(f"❌ ERRO: {exc}")

    if not dataframes:
        raise RuntimeError("Nenhuma série foi baixada com sucesso.")

    base = reduce(lambda l, r: pd.merge(l, r, on=DATE_COL, how="outer"), dataframes)
    base = base.sort_values(DATE_COL).reset_index(drop=True)

    out_path = DATA_DIR / "dados_consolidados_macro_credito.parquet"
    base.to_parquet(out_path, index=False)

    print(f"\n✅ Base consolidada salva em: {out_path}")
    print(f"   • Período: {base[DATE_COL].min().date()} a {base[DATE_COL].max().date()}")
    print(f"   • Observações: {len(base)}")
    print(f"   • Variáveis: {len(base.columns) - 1}")

    return base


# %% ==================== PREPARAÇÃO DOS DADOS ====================
def ensure_month_start(dates: pd.Series) -> pd.Series:
    """Garante que as datas estejam no primeiro dia do mês."""
    dates = pd.to_datetime(dates)
    return dates.values.astype("datetime64[M]").astype("datetime64[ns]")


def load_and_select() -> pd.DataFrame:
    """Carrega a base consolidada e seleciona target + features disponíveis."""
    input_file = DATA_DIR / "dados_consolidados_macro_credito.parquet"
    if not input_file.exists():
        raise FileNotFoundError(f"Arquivo não encontrado: {input_file}")

    df = pd.read_parquet(input_file).copy()
    df[DATE_COL] = ensure_month_start(df[DATE_COL])
    df = df.sort_values(DATE_COL).reset_index(drop=True)

    if TARGET not in df.columns:
        raise ValueError(f"Target '{TARGET}' não encontrada na base.")

    feats = [c for c in FEATURES_RAW if c in df.columns]
    missing = [c for c in FEATURES_RAW if c not in df.columns]

    if missing:
        (RES_DIR / "missing_features.txt").write_text(
            "Features configuradas ausentes na base:\n" + "\n".join(missing),
            encoding="utf-8"
        )

    selected = [DATE_COL, TARGET] + feats
    df = df[selected].copy()

    missingness = df.isna().mean().sort_values(ascending=False)
    (RES_DIR / "missingness_report.txt").write_text(
        "Proporção de valores ausentes por coluna:\n\n" + missingness.to_string(),
        encoding="utf-8"
    )

    return df


def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Constrói a base final sem data leakage:
        - features macro em nível
        - TARGET_lag1 como única defasagem
    """
    out = df.copy()

    if ADD_TARGET_LAG1:
        out[f"{TARGET}_lag1"] = out[TARGET].shift(1)

    out = out.dropna().reset_index(drop=True)
    return out


def make_scenarios(df_features: pd.DataFrame):
    """Gera cenários FULL e EXCL."""
    df_full = df_features.copy()

    mask_drop = (
        (df_full[DATE_COL] >= pd.to_datetime(EXCL_START)) &
        (df_full[DATE_COL] <= pd.to_datetime(EXCL_END))
    )
    df_excl = df_full.loc[~mask_drop].reset_index(drop=True)

    return df_full, df_excl


# %% ==================== FUNÇÕES AUXILIARES DOS DIAGNÓSTICOS ====================
def _safe_boxcox(x: np.ndarray):
    """
    Aplica Box-Cox com shift automático, caso haja valores não positivos.
    """
    x = np.asarray(x, dtype=float)
    shift = 0.0
    xmin = np.nanmin(x)
    if xmin <= 0:
        shift = abs(xmin) + 1e-6
        x = x + shift

    x_bc, lam = boxcox(x)
    return x_bc, float(lam), float(shift)


def _heatmap_corr(corr: pd.DataFrame, outpath: Path, title: str):
    """Gera heatmap de correlação usando matplotlib puro."""
    fig, ax = plt.subplots(figsize=(12, 10))
    im = ax.imshow(corr.values, aspect="auto")

    ax.set_xticks(np.arange(corr.shape[1]))
    ax.set_yticks(np.arange(corr.shape[0]))
    ax.set_xticklabels(corr.columns, rotation=90)
    ax.set_yticklabels(corr.index)

    for i in range(corr.shape[0]):
        for j in range(corr.shape[1]):
            ax.text(j, i, f"{corr.values[i, j]:.2f}", ha="center", va="center", fontsize=7)

    format_axes(ax, title=title)
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.ax.tick_params(labelsize=9)

    plt.tight_layout()
    plt.savefig(outpath, dpi=300, bbox_inches="tight")
    plt.close(fig)


def _scatter_matrix(df_small: pd.DataFrame, outpath: Path, title: str):
    """Gera scatter-matrix com pandas."""
    from pandas.plotting import scatter_matrix

    fig = plt.figure(figsize=(14, 14))
    axes = scatter_matrix(df_small, alpha=0.7, diagonal="hist", figsize=(14, 14))

    for ax in axes.flatten():
        for lab in ax.get_xticklabels() + ax.get_yticklabels():
            lab.set_fontfamily("Arial")
            lab.set_fontsize(7)

    plt.suptitle(title, fontfamily="Arial", fontsize=12, y=1.02)
    plt.tight_layout()
    plt.savefig(outpath, dpi=300, bbox_inches="tight")
    plt.close(fig)


def _residual_plots(resid: np.ndarray, out_hist: Path, out_qq: Path, title_prefix: str):
    """Gera histograma e QQ-plot dos resíduos."""
    resid = np.asarray(resid, dtype=float)
    mu, sigma = norm.fit(resid)

    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(resid, bins=20, density=True, alpha=0.6)
    x = np.linspace(resid.min(), resid.max(), 200)
    ax.plot(x, norm.pdf(x, mu, sigma), linewidth=2)

    format_axes(
        ax,
        xlabel="Resíduos",
        ylabel="Densidade",
        title=f"{title_prefix} — Histograma dos resíduos e Normal teórica",
        legend=False,
    )
    plt.tight_layout()
    plt.savefig(out_hist, dpi=300, bbox_inches="tight")
    plt.close(fig)

    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplot(111)
    sm.qqplot(resid, line="45", ax=ax)
    format_axes(
        ax,
        xlabel="Quantis teóricos",
        ylabel="Quantis amostrais",
        title=f"{title_prefix} — QQ-plot dos resíduos",
        legend=False
    )
    plt.tight_layout()
    plt.savefig(out_qq, dpi=300, bbox_inches="tight")
    plt.close(fig)


# %% ==================== DIAGNÓSTICOS ESTATÍSTICOS ====================
def run_diagnostics(df: pd.DataFrame, tag: str):
    """
    Executa diagnósticos estatísticos para o cenário informado.
    """
    print(f"\n=== Diagnósticos estatísticos ({tag}) ===")
    prefix = f"{tag}_"

    cols_num = [c for c in df.columns if c != DATE_COL]

    # Estatísticas descritivas
    desc = df[cols_num].describe().T
    desc.to_csv(RES_DIR / f"{prefix}describe.csv", encoding="utf-8")

    # Correlação
    corr = df[cols_num].corr(method="pearson")
    corr.to_csv(RES_DIR / f"{prefix}corr_matrix.csv", encoding="utf-8")
    _heatmap_corr(corr, RES_DIR / f"{prefix}corr_heatmap.png", f"{tag} — Matriz de correlação (Pearson)")

    if HAS_PINGOUIN:
        try:
            rcorr = pg.rcorr(df[cols_num], method="pearson", upper="pval", decimals=6)
            (RES_DIR / f"{prefix}corr_rcorr_pingouin.txt").write_text(str(rcorr), encoding="utf-8")
        except Exception as exc:
            (RES_DIR / f"{prefix}corr_rcorr_pingouin.txt").write_text(
                f"Falha ao calcular rcorr com pingouin: {exc}",
                encoding="utf-8"
            )
    else:
        (RES_DIR / f"{prefix}corr_rcorr_pingouin.txt").write_text(
            "pingouin não disponível no ambiente.",
            encoding="utf-8"
        )

    # Scatter matrix
    target_corr = corr[TARGET].drop(TARGET).abs().sort_values(ascending=False)
    top_feats = target_corr.head(min(TOPK_SCATTER, len(target_corr))).index.tolist()
    if top_feats:
        pairs = df[top_feats + [TARGET]].copy()
        _scatter_matrix(
            pairs,
            RES_DIR / f"{prefix}scatter_matrix_pairs.png",
            f"{tag} — Scatter-matrix (TOP {len(top_feats)} features vs Target)"
        )

    # VIF e tolerância
    feats = [c for c in cols_num if c != TARGET]
    X = df[feats].values
    scaler = StandardScaler()
    Xs = scaler.fit_transform(X)
    Xv = sm.add_constant(Xs)

    vif = pd.DataFrame({
        "Variavel": ["const"] + feats,
        "VIF": [variance_inflation_factor(Xv, i) for i in range(Xv.shape[1])]
    })
    vif["Tolerancia"] = 1.0 / vif["VIF"].replace(0, np.nan)
    vif.to_csv(RES_DIR / f"{prefix}vif_table.csv", index=False, encoding="utf-8")

    cond_number = np.linalg.cond(Xv)

    # Resíduos de baseline linear
    y = df[TARGET].values
    lin = LinearRegression()
    lin.fit(Xs, y)
    yhat = lin.predict(Xs)
    resid = y - yhat

    lines = []
    lines.append(f"=== NORMALIDADE DOS RESÍDUOS ({tag}) — baseline linear ===\n\n")
    lines.append(f"N observações: {len(resid)}\n")
    lines.append(f"Condition number (com constante): {cond_number:.4f}\n\n")

    try:
        sw_stat, sw_p = shapiro(resid)
        lines.append(f"Shapiro-Wilk: W={sw_stat:.6f}, p-value={sw_p:.6f}\n")
    except Exception as exc:
        lines.append(f"Shapiro-Wilk: falhou ({exc})\n")

    if HAS_SHAPIRO_FRANCIA:
        try:
            sf_items = shapiro_francia(resid).items()
            _, statistics_w, _, pval = sf_items
            lines.append(f"Shapiro-Francia: W={statistics_w[1]:.6f}, p-value={pval[1]:.6f}\n")
        except Exception as exc:
            lines.append(f"Shapiro-Francia: falhou ({exc})\n")
    else:
        lines.append("Shapiro-Francia: pacote 'statstests' não disponível.\n")

    jb_stat, jb_p, skew, kurt = jarque_bera(resid)
    lines.append(f"Jarque-Bera: JB={jb_stat:.6f}, p-value={jb_p:.6f}, skew={skew:.6f}, kurt={kurt:.6f}\n\n")
    lines.append("Interpretação:\n")
    lines.append("- H0: resíduos aderem à normalidade.\n")
    lines.append("- Rejeita-se H0 se p-value <= 0,05.\n")

    (RES_DIR / f"{prefix}residuals_normality_tests.txt").write_text("".join(lines), encoding="utf-8")

    _residual_plots(
        resid,
        out_hist=RES_DIR / f"{prefix}residuals_hist.png",
        out_qq=RES_DIR / f"{prefix}residuals_qqplot.png",
        title_prefix=tag
    )

    # Box-Cox da target
    try:
        _, lam, shift = _safe_boxcox(y)
        (RES_DIR / f"{prefix}boxcox_target_info.txt").write_text(
            "=== BOX-COX (TARGET) ===\n"
            f"Lambda estimado: {lam:.6f}\n"
            f"Shift aplicado: {shift:.6f}\n"
            "Obs.: Box-Cox exige valores estritamente positivos.\n",
            encoding="utf-8"
        )
    except Exception as exc:
        (RES_DIR / f"{prefix}boxcox_target_info.txt").write_text(
            f"Box-Cox falhou: {exc}",
            encoding="utf-8"
        )

    # Stepwise opcional
    if HAS_STEPWISE:
        try:
            formula = TARGET + " ~ " + " + ".join(feats)
            model = sm.OLS.from_formula(formula, df[cols_num]).fit()
            model_step = stepwise(model, pvalue_limit=STEPWISE_P)

            params = list(model_step.params.index)
            selected = [p for p in params if p not in ["Intercept", "const"]]

            (RES_DIR / f"{prefix}stepwise_selected_features.txt").write_text(
                "=== FEATURES SELECIONADAS (STEPWISE) ===\n"
                f"p-value limite: {STEPWISE_P}\n\n" +
                ("\n".join(selected) if selected else "(nenhuma feature selecionada)"),
                encoding="utf-8"
            )

            resid_sw = model_step.resid
            lines_sw = []
            lines_sw.append(f"=== NORMALIDADE DOS RESÍDUOS ({tag}) — STEPWISE ===\n\n")
            lines_sw.append(f"N observações: {len(resid_sw)}\n\n")

            try:
                sw_stat2, sw_p2 = shapiro(resid_sw)
                lines_sw.append(f"Shapiro-Wilk: W={sw_stat2:.6f}, p-value={sw_p2:.6f}\n")
            except Exception as exc:
                lines_sw.append(f"Shapiro-Wilk: falhou ({exc})\n")

            if HAS_SHAPIRO_FRANCIA:
                try:
                    sf_items2 = shapiro_francia(resid_sw).items()
                    _, statistics_w2, _, pval2 = sf_items2
                    lines_sw.append(f"Shapiro-Francia: W={statistics_w2[1]:.6f}, p-value={pval2[1]:.6f}\n")
                except Exception as exc:
                    lines_sw.append(f"Shapiro-Francia: falhou ({exc})\n")

            jb_stat2, jb_p2, skew2, kurt2 = jarque_bera(resid_sw)
            lines_sw.append(
                f"Jarque-Bera: JB={jb_stat2:.6f}, p-value={jb_p2:.6f}, "
                f"skew={skew2:.6f}, kurt={kurt2:.6f}\n"
            )

            (RES_DIR / f"{prefix}stepwise_residuals_normality_tests.txt").write_text(
                "".join(lines_sw),
                encoding="utf-8"
            )

            _residual_plots(
                resid_sw,
                out_hist=RES_DIR / f"{prefix}stepwise_residuals_hist.png",
                out_qq=RES_DIR / f"{prefix}stepwise_residuals_qqplot.png",
                title_prefix=f"{tag} (Stepwise)"
            )

        except Exception as exc:
            (RES_DIR / f"{prefix}stepwise_selected_features.txt").write_text(
                f"Stepwise falhou: {exc}",
                encoding="utf-8"
            )
    else:
        (RES_DIR / f"{prefix}stepwise_selected_features.txt").write_text(
            "Stepwise não disponível no ambiente.",
            encoding="utf-8"
        )

    print(f"✅ Diagnósticos de preparação salvos em: {RES_DIR}")


# %% ==================== EXECUÇÃO PRINCIPAL ====================
def main():
    print("\n" + "=" * 80)
    print("TCC - PREVISÃO DA INADIMPLÊNCIA DE CARTÕES DE CRÉDITO NO BRASIL")
    print("PIPELINE 1: EXTRAÇÃO, PREPARAÇÃO E DIAGNÓSTICOS")
    print("=" * 80)

    extract_all_series()

    print("\n" + "=" * 80)
    print("BLOCO 2: PREPARAÇÃO DOS DATASETS")
    print("=" * 80)

    df_raw = load_and_select()
    print(f"Linhas (raw): {len(df_raw)} | Colunas: {df_raw.shape[1]}")

    df_feat = build_features(df_raw)
    print(f"Linhas (features): {len(df_feat)} | Colunas: {df_feat.shape[1]}")
    print(f"Período: {df_feat[DATE_COL].min().date()} até {df_feat[DATE_COL].max().date()}")

    df_full, df_excl = make_scenarios(df_feat)
    print(f"Observações FULL: {len(df_full)}")
    print(f"Observações EXCL (sem 2019–2021): {len(df_excl)}")

    df_full.to_parquet(PREP_DIR / "prepared_FULL.parquet", index=False)
    df_excl.to_parquet(PREP_DIR / "prepared_EXCL.parquet", index=False)

    print(f"✅ Datasets preparados salvos em: {PREP_DIR}")

    print("\n" + "=" * 80)
    print("BLOCO 3: DIAGNÓSTICOS ESTATÍSTICOS")
    print("=" * 80)

    run_diagnostics(df_full, "FULL")
    run_diagnostics(df_excl, "EXCL")

    print("\n" + "=" * 80)
    print("✅ PIPELINE 1 CONCLUÍDO COM SUCESSO!")
    print("=" * 80)
    print("Arquivos gerados:")
    print(f"   1. {DATA_DIR / 'dados_consolidados_macro_credito.parquet'}")
    print(f"   2. {PREP_DIR / 'prepared_FULL.parquet'}")
    print(f"   3. {PREP_DIR / 'prepared_EXCL.parquet'}")
    print(f"   4. {RES_DIR}")
    print("\n🚀 Próximo passo: execute o script '02_analysis_pipeline.py'")
    print("=" * 80 + "\n")


if __name__ == "__main__":
    main()
